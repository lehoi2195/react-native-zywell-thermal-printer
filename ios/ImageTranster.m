...
#define RGBAMake(r, g, b, a)                                                   \
  (Mask8(a) | Mask8(b) << 8 | Mask8(g) << 16 | Mask8(r) << 24)
...
+ (UIImage *)covertToGrayScale:(UIImage *)image {
  ...
  CGContextRef context = CGBitmapContextCreate(
      pixels, width, height, 8, width * sizeof(uint32_t), colorSpace,
  ...
  for (int y = 0; y < height; y++) {
    for (int x = 0; x < width; x++) {
      ...
      intensity =
          (rgbaPixel[tt] + rgbaPixel[tt + 1] + rgbaPixel[tt + 2]) / 3. / 255.;
    ...
    for (NSInteger j = 0; j < h; j++) {
      for (NSInteger i = 0; i < w; i++) {
        ...
        if (i < w - 1 && j < h - 1) { // 不靠右边和下边的像素
          ...
          // 右下方像素处理
          //                         UInt8
          //                         leftlowPixel1=gradPixels[(w*j)+i+w+1]; int
          //                         llred1=Mask8(leftlowPixel1);
          //                        llred1+=e/4;
...
+ (uint8_t *)imgToGreyImage:(UIImage *)image {
  ...
  CGColorSpaceRef colorSpace = CGColorSpaceCreateDeviceRGB();
  CGContextRef context = CGBitmapContextCreate(
      rgbImage, actualWidth, actualHeight, 8, actualWidth * 4, colorSpace,
      kCGBitmapByteOrder32Little | kCGImageAlphaNoneSkipLast);
  CGContextSetInterpolationQuality(context, kCGInterpolationHigh);
  CGContextSetShouldAntialias(context, NO);
  CGContextDrawImage(context, CGRectMake(0, 0, actualWidth, actualHeight),
                     [image CGImage]);
  CGContextRelease(context);
  CGColorSpaceRelease(colorSpace);
 ...
 **Raster Image - $1D $76 $30 m xL xH yL yH d1...dk
 ...
 Notes:
 When ​standard mode​ is enabled, this command is only executed when there
 is no data in the print buffer. (Line is empty) The defined data (​d​)
 defines each byte of the raster image. Each bit in every byte defines a pixel.
 A bit set to 1 is printed and a bit set to 0 is not printed. If a raster bit
 image exceeds one line, the excess data is not printed. This command feeds as
 much paper as is required to print the entire raster bit image, regardless of
 line spacing defined by 1/6” or 1/8” commands. After the raster bit image is
 printed, the print position goes to the beginning of the line. The following
 commands have no effect on a raster bit image: Emphasized Double Strike
 Underline
 ...
 xL, xH ​defines the raster bit image in the horizontal direction in
 ​bytes​ using two-byte number definitions. (​xL + (xH * 256)) Bytes yL,
 yH ​defines the raster bit image in the vertical direction in ​dots​
 using two-byte number definitions. (​yL + (yH * 256)) Dots d ​ specifies
 the bit image data in raster format. k ​indicates the number of bytes in the
 bit image. ​k ​is not transmitted and is there for explanation only.
 **/
+ (NSData *)convertEachLinePixToCmd:(unsigned char *)src
  ...
  for (int i = 0; i < nHeight; i++) {
    ...
    for (int j = 0; j < nBytesPerLine; ++j) {
      data[var10 + 8 + j] =
          (int)(p_0[src[k]] + p_1[src[k + 1]] + p_2[src[k + 2]] +
                p_3[src[k + 3]] + p_4[src[k + 4]] + p_5[src[k + 5]] +
                p_6[src[k + 6]] + src[k + 7]);
      k = k + 8;
    }
  }
...
+ (NSData *)imgPixToTscCmd:(uint8_t *)src width:(NSInteger)width {
  ...
  for (int j = 0; k < length; ++k) {
    data[k] = (uint8_t)(p_0[src[j]] + p_1[src[j + 1]] + p_2[src[j + 2]] +
                        p_3[src[j + 3]] + p_4[src[j + 4]] + p_5[src[j + 5]] +
                        p_6[src[j + 6]] + src[j + 7]);
    j += 8;
  }
...
+ (UIImage *)imgPadLeft:(NSInteger)left withSource:(UIImage *)source {
  ...
  CGSize size =
      CGSizeMake(orgSize.width + [[NSNumber numberWithInteger:left] floatValue],
                 orgSize.height);
  UIGraphicsBeginImageContext(size);
